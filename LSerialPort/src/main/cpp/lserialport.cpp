...
extern "C"
JNIEXPORT jint JNICALL
Java_com_redrackham_LSerialPort_native_1sendMsg(JNIEnv *env, jobject thiz, jstring path, jbyteArray msg) {
    if (msg == nullptr) {
        return -1;
    }
    const char *path_char = env->GetStringUTFChars(path, nullptr);
...
extern "C"
JNIEXPORT jint JNICALL
Java_com_redrackham_LSerialPort_native_1setLSerialPortDataListener(JNIEnv *env, jobject thiz, jstring path,
                                                            jobject listener) {
    const char *path_char = env->GetStringUTFChars(path, nullptr);
    auto path_str = std::string(path_char);
